...
or start background work. Only the meeting owner can import one or two RTTM speakers; overlapping transcript
segments receive deterministic anonymous `Speaker A`/`Speaker B` labels. Repeating identical input is a no-op.
